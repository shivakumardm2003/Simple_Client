...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args[1].as_str() {
        "--mode=cache" => {
            println!("Choosing mode: Caching...");
            cache_mode(&args).await?;
        }
        "--mode=read" => {
            println!("Choosing mode: Reading...");
            read_mode().await?;
        }
        _ => {
...
async fn cache_mode(args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    writeln!(&mut file, "Cache complete. Average USD price of BTC on Coinbase is: {}", average)?;

    println!("Cache complete. Average USD price of BTC on Coinbase is: {}", average);
